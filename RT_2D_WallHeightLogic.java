...
// Getting closer to a DOOM foundation 
...
public class RT_2D_WallHeightLogic
{
    static enum AXIS { Xpos, Xneg, Ypos, Yneg }
    
    static final double FieldOfVision = 150;
    ...
    static Color [][] Frame = new Color[FrameHeight][FrameWidth];
    static ArrayList<Point> Intersections = new ArrayList<Point>();
    static ArrayList<Wall> Walls = new ArrayList<Wall>();       // this will become the Environment object
    ...
    static class TD_ViewPanel extends JPanel  
    {
        TD_ViewPanel()
        {
            addMouseListener(new WallCreator());
        }
        public void paintComponent(Graphics g)
        {
         ...
         //       System.out.println("Drawing Ray " + i + " from " + rayX1 + ", " + rayY1 + "  to  " + rayX2 + ", " + rayY2);
            }
            g.setColor(Color.black);
            for (Wall w : Walls)
                g.drawLine(w.endpoint1.X, w.endpoint1.Y, w.endpoint2.X, w.endpoint2.Y);
        ...
        public void paintComponent(Graphics g)
        {
            for (int y = 0; y < FrameHeight; ++y)
                for (int x = 0; x < FrameWidth; ++x)
                {
                    ...
                    g.fillRect(x * colorDistance, y, colorDistance, 1);
                //    System.out.println("COLOR : " + Frame[y][x]);
                //    System.out.println("Filling rectangle from " + (x*colorDistance) + ", " + y + " to " + (x*colorDistance+colorDistance) + ", " + (y+1));
                }
        }
    }
    ...
    static class WallCreator implements MouseListener
    {
        public void mouseClicked(MouseEvent e)
        {
            ...
            {
                Walls.add(new Wall(L1, new Point(X,Y), Color.black));
                L1 = null;
                td_view.repaint();
            }   
        }
    ...
    static class Wall extends LineSegment
    {
        Color color;
        Wall(Point p0, Point p1, Color c)
        {
            super(p0, p1);
            color = c;
        }
    ...
    static class Camera
    {
        ...
        Camera()
        {
            ...
            double initialRotation = toRadians( (  FieldOfVision / 2 ) );
            double incrementalRotation = toRadians( (FieldOfVision / (FrameWidth-1)) );
        ...
        //        System.out.println("Ray " + i + " " + Rays[i].toString());
            }    
        ...
        void updateFrame()
        {
            ...
            for (int x = 0; x < Rays.length; ++x)
            {
                ...
                for (Wall w : Walls)
                {
                    Point p = findIntersection(Rays[x], w);
                    if (p == null) continue;
                    if ( withinEndpoints(p, w) ) 
                    {
                        if (pointBuffer == null)
                        {
                            pointBuffer = p; 
                            pointBuffer.setWall(w);
                            Intersections.add(p);
                        }
                        else if (distance(p, camera.origin) < distance(pointBuffer, camera.origin)) 
                        {
                            pointBuffer = p;   
                            pointBuffer.setWall(w);
                            Intersections.add(p);
        ...
        void colorLineOfFrame(int rayX)
        {
            if (pointBuffer == null)
                for (int y = 0; y < FrameHeight; ++y) 
                    Frame[y][rayX] = defaultColor;
            else 
            {
                int d = toInt( ( distance(pointBuffer, camera.origin) / 2 ) );
             //   System.out.println("rayX = " + rayX);
             //   System.out.println("d = " + d);
                
                for (int y = 0; y < d; ++y)
                {
                    Frame[y][rayX] = defaultColor; // System.out.println("Coloring Frame[" + y + "][" + rayX + " ] " + defaultColor);
                }
                
                for (int y = d; y < FrameHeight - d; ++y)
                {
                    Frame[y][rayX] = pointBuffer.wall.color; //   System.out.println("Coloring Frame[" + y + "][" + rayX + " ] " + pointBuffer.wall.color);
                }
                for (int y = FrameHeight - d; y < FrameHeight; ++y)
                {
                    Frame[y][rayX] = defaultColor; // System.out.println("Coloring Frame[" + y + "][" + rayX + " ] " + pointBuffer.wall.color);
                } 
            }
        }
